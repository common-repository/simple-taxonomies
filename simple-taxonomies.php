...
if ( ! class_exists( 'YoastSimpleTaxonomies_Admin' ) ) {
	...
	class YoastSimpleTaxonomies_Admin {
		...
		function filter_plugin_actions( $links, $file ){
			...
			if ( $file == $this_plugin ){
				$settings_link = '<a href="edit.php?page='.basename( __FILE__ ).'">' . __('Settings', 'simple-taxonomies') . '</a>';
		...
		function config_page() {
			if ( isset($_POST['submit']) ) {
				if (!current_user_can( 'manage_options' )) die(__( 'You cannot edit the Yoast Simple Taxonomies options.' , 'simple-taxonomies'));
				...
				echo '<div id="message" class="updated"><p>'.__('Yoast Simple Taxonomies settings updated.', 'simple-taxonomies').'</p></div>';
			}
			$opt  = get_option( 'yoast_simpletax' );
			?>
			<div class="wrap">
				<?php screen_icon(); ?>
				<h2><?php _e("Yoast Simple Taxonomies Configuration", 'simple-taxonomies'); ?></h2>
				<form action="" method="post" id="yoastsimpletax-conf">
					...
					<table class="form-table">
						...
						<tr>
							<th scope="row" style="vertical-align:top;width:15%;"><?php if (isset($taxonomy['label'])) { echo '<a id="'.$taxonomy['name'].'" href="'.admin_url("edit-tags.php?taxonomy=".$taxonomy['name']).'">'.$taxonomy['label'].'</a>'; } else { _e("New Taxonomy", 'simple-taxonomies'); } ?>:</th>
							<td style="vertical-align:top;width:85%;">
								<table class="form-table" width="100%">
									<tr>
										<th style="vertical-align:top;width:35%;">
											<label for="name<?php echo $i; ?>"><?php _e("Name / Slug", 'simple-taxonomies'); ?>:</label><br/>
											<small>(<?php _e("All lowercase and no weird characters", 'simple-taxonomies'); ?>)</small>
										</th>
									...
									<tr>
										<th style="vertical-align:top;">
											<label for="label<?php echo $i; ?>"><?php _e("Label", 'simple-taxonomies'); ?>:</label><br/>
											<small>(<?php _e("The label used in the backend, on widgets etc.", 'simple-taxonomies'); ?>)</small>
										</th>
									...
									<tr>
										<th style="vertical-align:top;"><label for="filter<?php echo $i; ?>"><?php _e("Add terms to the end of posts", 'simple-taxonomies'); ?>:</label></th>
									...
									<tr>
										<th style="vertical-align:top;"><label for="filterexcerpt<?php echo $i; ?>"><?php _e("Add terms to the end of excerpts", 'simple-taxonomies'); ?>:</label></th>
					...
					<table class="form-table">
						<tr>
							<th style="width:60%;">
								<?php _e('Disable the "post_tag" taxonomy?', 'simple-taxonomies'); ?><br/>
								<small><?php _e('If you have defined other taxonomies, you might think the "normal" tag taxonomy is not needed, this removes it.', 'simple-taxonomies'); ?></small>
							</th>
							<td>
					...
					<br/>
					<span class="submit" style="border: 0;"><input type="submit" name="submit" value="<?php _e("Save Settings", 'simple-taxonomies'); ?>" /></span>
				</form>
			</div>
<?php		}
	}
...
class Yoast_Taxonomy_Widget extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e("Title", 'simple-taxonomies'); ?>:</label>
			<input id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" style="width:100%;" />
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'taxonomy' ); ?>"><?php _e("What to show", 'simple-taxonomies'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php _e("How to show it", 'simple-taxonomies'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'cloudorder' ); ?>"><?php _e("Order within cloud", 'simple-taxonomies'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'listorder' ); ?>"><?php _e("Order within list", 'simple-taxonomies'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'showcount' ); ?>"><?php _e("Show post count in list", 'simple-taxonomies'); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e("Number of tags to show", 'simple-taxonomies'); ?>:</label>
...
function yoast_taxonomy_edit_message() {
	...
	foreach ( $opt['taxonomies'] as $tax ) {
		if ( $tax['name'] == $_GET['taxonomy'] ) {
			...
			echo '<p><strong>'.__('Just a reminder', 'simple-taxonomies').'</strong>: '.__('this taxonomy was created with the Yoast Simple Taxonomies plugin, if you want, you can go', 'simple-taxonomies').' <a href="'.admin_url('edit.php?page='.basename(__FILE__)).'#'.$tax['name'].'">'.__('edit its settings', 'simple-taxonomies').'</a>.</p>';
			echo '</div><br/>';
		}
	}
}
